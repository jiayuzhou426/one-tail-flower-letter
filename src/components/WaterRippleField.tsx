...
type WaterRippleFieldProps = {
  ...
  onComplete?: () => void;
  className?: string;
  /** External water interactions in scene-relative percent coordinates. */
  pulses?: readonly WaterRipplePulse[];
  /** Leave hit testing to the parent when the canvas sits below DOM controls. */
  interactive?: boolean;
  /** A pond can reuse the GPU water without drawing the voyage fish. */
  showFish?: boolean;
  /** Keep a few subtle background disturbances in an otherwise static scene. */
  ambient?: boolean;
  /** Match the source image's CSS object-position when this is a static scene. */
  objectPosition?: 'center' | 'top';
};

export type WaterRipplePulse = {
  id: string;
  left: number;
  top: number;
  radius?: number;
...
export function WaterRippleField({
  ...
  const externalPulsesRef = useRef<WaterRipplePulse[]>([]);
  ...
  useEffect(() => {
    ...
    for (const pulse of pulses ?? []) {
      ...
      externalPulsesRef.current.push(pulse);
    }
    for (const id of seenPulseIdsRef.current) {
      if (!activeIds.has(id)) seenPulseIdsRef.current.delete(id);
    }
  }, [pulses]);
  ...
  useEffect(() => {
    ...
    const updateCoverTransform = (rect: DOMRect) => {
      ...
      if (displayAspect > sourceAspect) {
        ...
        imageOffsetY = objectPosition === 'top' ? 0 : playable ? imageOffsetRangeY : imageOffsetRangeY * .5;
      } else {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (!playable || !interactive) return;
    ...
    const drawScene = (fishPositionX: number, fishPositionY: number, fishBend: number, fishPhase: number) => {
      ...
      gl.uniform2f(sceneFishPosition, showFish ? fishPositionX : -2, showFish ? fishPositionY : -2);
    ...
    const render = (now: number) => {
      ...
      while (externalPulsesRef.current.length) {
        const pulse = externalPulsesRef.current.shift()!;
        const pulseX = Math.max(0, Math.min(1, pulse.left / 100));
        const pulseY = Math.max(0, Math.min(1, pulse.top / 100));
        if (waterAtScreenPoint(pulseX, pulseY)) {
          enqueueDrop(pulseX, pulseY, pulse.radius ?? (playable ? 20 : 34), pulse.strength ?? (playable ? .14 : .23));
        }
      }
      ...
      ambientPulse -= delta;
      if (ambient && !collisionDebug && ambientPulse <= 0) {
  ...
  }, [ambient, collisionDebug, durationMs, fishWake, fishX, fishY, imageSrc, interactive, maskSrc, objectPosition, playable, showFish]);
